...
- На момент этого файла изменения ещё не закоммичены и не запушены.
...
- После push дождаться Vercel deployment и сделать canary.
...
1. Проверить документацию на непротиворечивость и `git diff --check`.
2. Просмотреть точный список файлов; не использовать `git add -A`.
3. Сделать осмысленный task-коммит.
4. Rebase на свежий `origin/main`, если он изменился, и повторить гейт при
   конфликте или изменении продуктового среза.
5. Push ветки с `--force-with-lease` только если rebase потребует переписать
   уже опубликованную ветку; обычный первый push — без force.
6. Передать Gemini имя ветки и S3-2; PR/merge — после UI-фазы и зелёного гейта.

## Критерий завершения

Текущий CODEX/Checker-раунд завершён по коду и тестам. Передача завершится после
чистого diff, коммита и push task-ветки. Общая beta-ready цель не завершена до
production-flow двумя аккаунтами, настройки Auth URL/SMTP и исправления S3-2.
