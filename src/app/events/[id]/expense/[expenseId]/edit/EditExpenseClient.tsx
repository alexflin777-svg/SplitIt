...
import { CURRENCIES, convertCurrency, formatMoney, fetchLiveExchangeRates } from '@/lib/currency';
...
export default function EditExpenseClient({ groupId, expenseId }: { groupId: string; expenseId: string }) {
  ...
  useEffect(() => {
    ...
    const foundGroup = saved.find((g: any) => g.id === groupId);
    if (foundGroup) {
      setGroup(foundGroup);
      ...
      if (foundExp) {
        ...
        setCategory(foundExp.category || 'food');
  ...
  if (!group || !expense) {
    return (
      <div className="p-8 text-center space-y-4">
        ...
        <Link href={`/events/${groupId}`} className="text-xs font-bold text-blue-600 underline">
          Вернуться к событию
        </Link>
      </div>
    );
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const updatedGroup = {
      ...group,
      expenses: (group.expenses || []).map((e: any) => (e.id === expenseId ? updatedExpense : e)),
    };
  ...
  const handleDeleteConfirmed = () => {
    const updatedGroup = {
      ...group,
      expenses: (group.expenses || []).filter((e: any) => e.id !== expenseId),
    };
