...
| S2-9 | При прогонах с перенаправлением вывода в конвейер сводка Playwright несколько раз напечатала меньше тестов, чем в наборе: «95 passed» вместо 96 и «12/13 passed» вместо 14 — без единого failed, interrupted или flaky. Отличить артефакт рендеринга list-репортера от реальной потери тестов по выводу нельзя. При записи вывода в файл и с `--workers=1` расхождений нет; exit code всегда 0. | S2 | наблюдение при повторных прогонах 15.08 | 🟡 Причина не установлена, и это принципиально: сводка сама по себе доказательством считаться больше не может. Закрыто сторожем `e2e/reporters/expect-all-tests.ts` — он сравнивает собранные тесты с отчитавшимися и валит прогон при расхождении. Сторож проверен красным (6 недостающих, exit 1) и зелёным (14 passed) |
